...
func CreatePacmanDb(mds []*DescParams) ([]byte, error) {
	entries := make(map[string][]byte)

	for _, md := range mds {
		entries[md.Name+"-"+md.Version+"/desc"] = []byte(md.Desc)
	}
...
// Eject version and name from package file name/sign/desc.
func EjectNameParameters(name string) (string, string, error) {
	splt := strings.Split(name, "-")
	if len(splt) < 3 {
		return ``, ``, errors.New("invalid package name")
	}
	l := len(splt)
	return strings.Join(splt[0:l-3], "-"), strings.Join(splt[l-3:l-1], "-"), nil
}
